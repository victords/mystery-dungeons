...
function EditorScene.new(id)
  ...
  self.canvas = love.graphics.newCanvas(SCENE_DISPLAY_WIDTH, SCENE_DISPLAY_HEIGHT)
  return self
end
...
function EditorScene:draw()
  ...
  for i = 1, TILES_X do
    for j = 1, TILES_Y do
      if self:is_wall(i, j) then Window.draw_rectangle((i - 1) * TILE_SIZE, (j - 1) * TILE_SIZE, TILE_SIZE, TILE_SIZE, {1, 1, 0, 0.32}) end
      if self.tiles[i][j] == -1 then Window.draw_rectangle((i - 1) * TILE_SIZE, (j - 1) * TILE_SIZE, TILE_SIZE, TILE_SIZE, {0, 1, 0, 0.32}) end
    end
  end

  for index, e in ipairs(self.entrances) do
    local x = (e[1] - 1) * TILE_SIZE
    local y = (e[2] - 1) * TILE_SIZE
    Window.draw_rectangle(x, y, TILE_SIZE, TILE_SIZE, {0, 0, 1})
    Editor.font:draw_text(index, x, y)
  end
  for index, e in ipairs(self.exits) do
    local x = (e.col - 1) * TILE_SIZE
    local y = (e.row - 1) * TILE_SIZE
    Window.draw_rectangle(x, y, TILE_SIZE, TILE_SIZE, {1, 0, 0})
    Editor.font:draw_text(e.dest_scene .. "," .. e.dest_entrance, x, y)
  end
...
function EditorScene:add_object(col, row, class_name, args)
  ...
  table.insert(self.objects, _G[class_name].new(col, row, Utils.split(args, ",")))
end
...
function EditorScene:serialize()
  local lines = {
    ...
    Utils.join(Utils.map(self.objects, function(o) return o.class .. "," .. o.col .. "," .. o.row .. "," .. (o.args and Utils.join(o.args, ",") or "") end), "|"),
  }
  for j = 1, TILES_Y do
    local line = ""
    for i = 1, TILES_X do
...
function EditorScene:tile_char_from_number(tile)
...
Editor = {
  init = function ()
    ...
    Editor.lbl_tool = Label.new(SCENE_DISPLAY_WIDTH + 10, 10, {font = font, text = "[none]"})
    Editor.lbl_dest_scene = Label.new(50, 210, {font = font, text = "1", anchor = "top_right"})
    Editor.lbl_dest_entr = Label.new(50, 252, {font = font, text = "1", anchor = "top_right"})
    Editor.lbl_obj_name = Label.new(50, 344, {font = font, text = Editor.object_names[1], anchor = "top_right"})
    Editor.txt_obj_args = TextField.new(10, 386, {anchor = "top_right", font = font, h = 32})
    Editor.lbl_level = Label.new(50, 428, {font = font, text = "1", anchor = "top_right"})

    Editor.controls = {
      Editor.lbl_tool,
      Button.new(10, 10, {w = 40, h = 40, anchor = "top_right", font = font, text = '#'}, function () Editor.set_tool("wall") end),
      Button.new(10, 60, {w = 40, h = 40, anchor = "top_right", font = font, text = '/'}, function () Editor.set_tool("wall_edge") end),
      Button.new(10, 110, {w = 40, h = 40, anchor = "top_right", font = font, text = 'e'}, function () Editor.set_tool("entrance") end),
      Button.new(10, 160, {w = 40, h = 40, anchor = "top_right", font = font, text = 'x'}, function () Editor.set_tool("exit") end),
      Editor.lbl_dest_scene,
      Button.new(10, 210, {w = 32, h = 32, anchor = "top_right", font = font, text = '>'}, function ()
        Editor.exit_dest_scene = Editor.exit_dest_scene + 1
        Editor.lbl_dest_scene:set_text(tostring(Editor.exit_dest_scene))
      end),
      Button.new(90, 210, {w = 32, h = 32, anchor = "top_right", font = font, text = '<'}, function ()
      ...
      Button.new(10, 252, {w = 32, h = 32, anchor = "top_right", font = font, text = '>'}, function ()
        Editor.exit_dest_entr = Editor.exit_dest_entr + 1
        Editor.lbl_dest_entr:set_text(tostring(Editor.exit_dest_entr))
      end),
      Button.new(90, 252, {w = 32, h = 32, anchor = "top_right", font = font, text = '<'}, function ()
        if Editor.exit_dest_entr > 0 then
          Editor.exit_dest_entr = Editor.exit_dest_entr - 1
          Editor.lbl_dest_entr:set_text(tostring(Editor.exit_dest_entr))
        end
      end),
      Button.new(10, 294, {w = 40, h = 40, anchor = "top_right", font = font, text = 'o'}, function () set_tool("object") end),
      Editor.lbl_obj_name,
      Button.new(10, 344, {w = 32, h = 32, anchor = "top_right", font = font, text = '>'}, function ()
        Editor.object_index = (Editor.object_index + 1) % Editor.object_names.size
        Editor.lbl_obj_name:set_text(Editor.object_names[Editor.object_index])
        Editor.txt_obj_args:set_text("")
      end),
      Button.new(180, 344, {w = 32, h = 32, anchor = "top_right", font = font, text = '<'}, function ()
        Editor.object_index = (Editor.object_index - 1) % #Editor.object_names
        Editor.lbl_obj_name:set_text(Editor.object_names[Editor.object_index])
        Editor.txt_obj_args:set_text("")
      end),
      Editor.txt_obj_args,
      Editor.lbl_level,
      Button.new(10, 428, {w = 32, h = 32, anchor = "top_right", font = font, text = '>'}, function ()
        Editor.level_id = Editor.level_id + 1
        Editor.lbl_level:set_text(tostring(Editor.level_id))
      end),
      Button.new(90, 428, {w = 32, h = 32, anchor = "top_right", font = font, text = '<'}, function ()
        if Editor.level_id > 1 then
          Editor.level_id = Editor.level_id - 1
          Editor.lbl_level:set_text(tostring(Editor.level_id))
        end
      end),
      Button.new(10, 470, {h = 40, anchor = "top_right", font = font, text = 'Save'}, function ()
        print("save")
        --$gtk.write_file("data/scene/#{Editor.level_id}.txt", Editor.scene.serialize)
      end),
      Button.new(10, 520, {h = 40, anchor = "top_right", font = font, text = 'Load'}, function ()
